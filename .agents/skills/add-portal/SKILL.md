---
name: add-portal
description: Generates a job-portal search skill (SKILL.md, url-reference.md, CLI with tests) for a job board in a given market, following the same zero-dependency pattern as the existing linkedin-search/jobindex-search/etc skills. Investigates the portal, scaffolds the skill, and test-runs a live query before registering anything. Use for "/add-portal", "/add-portal --list", or "/add-portal <portal URL>".
user-invocable: true
allowed-tools: read_file write_file edit grep bash git_bash web_fetch ask_user_question
---

# /add-portal - Generate a Job-Portal Search Skill for Your Local Market

You are helping the user build a job-portal search skill for a job board in their market. The repo ships worked examples of the pattern (four Danish portals plus the country-agnostic `linkedin-search`), and this command turns that pattern into a guided workflow: investigate the portal, scaffold the skill from the canonical structure, and test-run a live query before registering anything.

The generator is **country-agnostic**: it works for any portal in any market and language. The skills it produces are typically market-specific and stay local to this workspace.

The skill argument may contain a subcommand, a portal URL, or nothing.
...
- If the argument contains a URL: treat it as the portal URL and carry it into Step 1.
- Otherwise: start the interview at Step 1.

### List Mode

Run `bash`/`git_bash`: `find .agents/skills -maxdepth 1 -name "*-search" -type d` to find installed portal skills. For each, read `SKILL.md` and print a table:

```
## Registered Portal Skills

| Name | Market | Data source |
|------|--------|-------------|
| <name> | <market from description> | <data source from url-reference.md> |
```
...
2. **Skill name** - kebab-case, suffixed `-search` (e.g. `seek-search`, `stepstone-search`). Must not collide with an existing folder in `.agents/skills/` (check with `bash`/`git_bash` `ls .agents/skills/`).
...
Do reconnaissance before writing any code. Use `web_fetch` (or `curl` via `bash`/`git_bash`) on the portal:

1. **Find the search URL pattern.** Load the portal's search page, identify: the search endpoint, the query parameter, and any parameters for location, posting age, and pagination. Prefer a JSON API if one backs the site (check for `/api/` XHR endpoints in the page source); otherwise plan to parse the HTML results page.
...
These conventions are what make portal skills interchangeable for `rank` and for users reading any skill's docs:
...
- **`SKILL.md` frontmatter:** `name`, `version: 1.0.0`, a `description` written for skill triggering - it must name the portal, the market, and include trigger phrases in English **and** the market's language; `allowed-tools: Bash(bun run .agents/skills/<name>/cli/src/cli.ts *)` (matching the pattern the existing portal skills use, which this workspace has confirmed Vibe discovers correctly).
...
1. Ask whether the user wants the new portal added to their scraping search strategy. If yes, resolve the search-queries file first - personal data first, generic fallback otherwise:
   ```bash
   ...
   ```
   Add the portal's site to the relevant query categories in the **resolved path** (site-specific queries, like the existing `jobindex.dk` entries) so the `scrape` skill includes it. If `resolvedPath` fell back to the generic file (no `personal/` file exists yet), still write to `personal/job-scraper-search-queries.md` instead - creating it if needed - never to the generic tracked template, which would commit real search-strategy data into a git-tracked file.
2. Remind the user to run the install line for their own records:
   ```bash
   cd .agents/skills/<name>/cli && bun install && cd ../../../..
   ```
   (Skip if the skill is zero-dependency and they don't care about typecheck types.)
3. Note that the skill auto-triggers from its `SKILL.md` description - no other wiring is needed, matching how `linkedin-search` and the four Danish portal skills already work in this workspace.
...
- The generator is country-agnostic; its output is market-specific and stays local to this workspace.
