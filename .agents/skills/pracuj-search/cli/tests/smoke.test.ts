...
// Live smoke tests - hit the real pracuj.pl site. Keep volume low: one search, one detail.

describe("live smoke test", () => {
  test("search with a realistic query returns at least one real result", async () => {
    const result = await runCLI(["search", "-q", "software asset manager", "--limit", "5", "--format", "json"]);
    ...
    expect(job.url).toContain("pracuj.pl");
  }, 30000);

  test("detail on a real ID returns a description", async () => {
    const search = await runCLI(["search", "-q", "software asset manager", "--limit", "1", "--format", "json"]);
