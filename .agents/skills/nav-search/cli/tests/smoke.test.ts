...
// Live smoke tests - hit the real NAV Arbeidsplassen API/site. Keep volume low.
...
describe("live smoke test", () => {
  test("search with a realistic query returns at least one real result", async () => {
    ...
    expect(job.url).toContain("arbeidsplassen.nav.no");
  }, 30000);
  ...
  test("detail on a real ID returns a description", async () => {
    ...
    expect(job.title).toBeTruthy();
    // Some listings genuinely have no parseable description chunk - only assert
    // the command succeeded and returned a title, not that description is non-null.
  }, 30000);
