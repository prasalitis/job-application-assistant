...
// Live smoke tests - hit the real moovijob.com site. Keep volume low: one search, one detail.
...
describe("live smoke test", () => {
  test("search with a realistic query returns at least one real result", async () => {
    ...
    expect(job.url).toContain("moovijob.com");
  }, 30000);

  test("detail on a real result returns a description", async () => {
    const search = await runCLI(["search", "-q", "IT governance", "--limit", "1", "--format", "json"]);
    const { results } = parseJSON<{ results: Array<{ url: string }> }>(search);
    expect(results.length).toBeGreaterThan(0);

    const detail = await runCLI(["detail", results[0].url, "--format", "json"]);
