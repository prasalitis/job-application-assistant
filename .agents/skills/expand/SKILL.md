---
name: expand
description: Additive-only competency discovery from documents/ (CV, LinkedIn export, diplomas, references) and public online presence (GitHub, portfolio sites). Extends the candidate and behavioral profile without ever modifying existing content. Use for "/expand".
user-invocable: true
...
## Step 0: Resolve and Read Existing Profile Files
...
Then use `ask_user_question` to ask:
...
Apply only the confirmed items. **Write to the resolved paths from Step 0, not the hardcoded `.agents/` filenames.** Use `edit` to add to the relevant sections of each file - do not rewrite entire files.

If Step 0's resolution for either file returned `usedPrimary: false` (no `personal/` file existed yet - a fresh clone with only the generic template), do not edit the generic tracked file. Instead create the personal file now with `write_file` at `personal/01-candidate-profile.md` / `personal/02-behavioral-profile.md`, seeded with the generic file's structure plus the new additions - this keeps personal data out of git from the very first `/expand` run, not just after a `/setup` has already created it.

### Additions to the candidate profile file
...
### Additions to the behavioral profile file
